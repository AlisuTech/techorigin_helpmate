...
const createNewServiceProvider = asyncHandler(async (req, res) => {
    const { firstName, lastName, phoneNumber, email, dateOfBirth, country, stateOfOrigin, password, photo, officeAddress, price, rating, yearsOfExperience, clientReviews, meansOfId, validIdFile, certificateFile} = req.body
...
const getAllServiceProviders = asyncHandler(async (req, res) => {
    const serviceProviders = await ServiceProvider.find().select('-password -confirmPassword').lean();
